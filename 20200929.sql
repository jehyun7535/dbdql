...
LEAD(sal) over
(ORDER BY sal desc, hiredate) lead_sal
FROM emp;
...
LAG(sal) over
(PARTITION BY job ORDER BY sal DESC, hiredate ASC) lag_sal
FROM emp;

이전/이후 n행 값 가져오기
LAG(col, 건널 뛸 행 수 - default 1, 값이 없을경우 적용할 기본값)
SELECT empno, ename, hiredate, job, sal,
LAG(sal, 2, 0) over
(ORDER BY sal DESC, hiredate ASC) lag_sal
FROM emp;
...
현재행 이전의 모든 행부터 ~ 현재까지 ==> 행들을 정렬할 기준이 있어야한다
...
SUM(sal) OVER (ORDER BY sal ROWS  UNBOUNDED PRECEDING ) range_sum,
...
후행 트랜잭션이 선행 트랜잭션에 어떻게 영향을 미치는지 정의한 단계

LEVEL 0~3
LEVEL 0 : READ UNICOMMITTED
          선행 트랜잭션이 커밋하기 않은 데이터도 후행 트랜잭션에서 조회된다
...
LEVEL 2 : REPEATABLE READ (반복적 읽기)
          ...
          오라클에서 공식적으로 지원하지는 않지만 SELECT FOR UPDATE 구문을 통해
          효과를 낼 수 있다
