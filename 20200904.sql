...
EX : emp 테이블에는 comm컬럼의 값이 NULL인 데티어가 존재
...
사원중에 자신의 상급자가 존재하지 않는 사원들만 조회(모든컬럼) 상급자 = MGR
SELECT *
FROM emp
WHERE MGR  IS NULL
...
emp 테이블의 사원중에 mgr가 7698이 아니거나, 7839가 아닌 사원
...
아래 sql 의 실행결과, 데이터의 조회 순서는 보장되지 않는다
지금은 7369, 7499.... 조회가 되지만 내일 동일한 sql을 실행 하더라도
오늘 순서가 보장되지 않는다 (바뀔수 있다)
