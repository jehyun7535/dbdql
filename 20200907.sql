...
SELECT empno, ename, LENGTH('hello'), LENGTH('hello')
FROM emp;
...
문자열 관련함수 
    ...
    TRUNC : 버림 함수
    ==> 몇번째 자리에서 반올림, 버림을 할지?
    두번째 인자가 0, 양수 : ROUND(숫자, 반올림 결과 지리수)
    ...
    SELECT ROUND(105.54, 1) trunc,
           ROUND(105.55, 1) trunc2,
           ROUND(105.55, 0) trunc3,
           ROUND(105.55, -1) trunc4
    FROM dual;
    ...
    날짜 관련함수
    문자열==> 날짜 타입 TO_DATE
    SYSDATE : 오라클 서버의 현재 날짜, 시간을 돌려주는특수함수
              함수의 인자가없다
              (java
               public void test()
               }
