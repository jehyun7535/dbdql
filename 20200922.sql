...
1. 데이터 노출을 방지
    (emp 테이블의 sal, comm을 제외하고 view를 생성, HR계정에게
        ...
        ==> V_EMP에는 sal, com컬럼 없기 떄문에 급여 관련 정보를 감출 수 있었다)
        
2. 자주 사용되는 쿼리를 view 만들어서 재사용
    ex: emp, 테이블은 dept 테이블이랑 조인되서 사용되는 경우 많음
...
SELECT emp.*, dept.ename, dept.loc
FROM emp, dept
...
CREATE SEQUENCE "JEHYUN"."SEQ_EMP"
MINVALUE 1 
...
SELECT *
FROM emp
WHERE ROWID= 'AAAE5hAAFAAAACLAAP';
...
PRIMARY KEY(UNIQUE + NOT NULL), UNIQUE 제약을 생성하면
해당 컬럼으로 인덱스를 생성한다
...
시나리오 2
emp 테이블의 empno 컬럼에 PRIMARY KEY 제약조건이 걸려 있는 겨웅
...
NON-UNIQUE 인덱스 : 인덱스 구성 컬럼의 중복 값을 허용하는 인덱스
(emp.deptno, emp.job)

시나리오3
emp테이블에 empno컬럼에 non-unique인덱스가 있는경우
