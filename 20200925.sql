...
GROUP BY 의 확장 : SUBGROUP을 자동을 생성하여 하나의 결과로 합쳐준다
1. ROLLUP(col1, col2..)
    . 기술된 컬럼을 오른쪽에서부터 지워 나가며 서브 그룹을 생성
...
CUBE의 경우 가능한 모든 조합으로 서브 그룹을 생성하기 떄문에
...
3. EMP TEST테이블에는 DNAME 컬럼을 추가( VARCHAR2(14) )
4. 상호 연관 서브쿼리를 이용하여  EMP_TEST테이블의 DNAME 컬럼을 DEPT을 이용하 UPDATE
...
UPDATE emp_test SET dname = (SELECT dname FROM dept 
...
DROP TABLE emp_test; --테이블 있으면 삭제--

ALTER TABLE dept_test ADD (empcnt NUMBER); --컬럼 추가--
...
달력만들기 : 행을 열로 만들기-레포트 쿼리에서 자주 사용하는 현태
주어진것 : 년월 (수업시간에는 '202009' 문자열을 사용)

'202009' ==> 30
'202008' ==> 31
SELECT TO_DATE('202002', 'YYYYMM') + LEVEL -1
SELECT TO_CHAR(LAST_DAY(TO_DATE('202011', 'YYYYMM')), 'DD')
FROM dual;
