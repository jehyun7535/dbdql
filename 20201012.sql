...
3. 속성 정의
    ...
    파생 속성 : 다른 속성으로 부터 계산된 값
               게시글의 첨부파일 개수
               엔터티 : 게시글, 첨부파일
                        해당 게시글 첨부파일 수는 첨부 파일 엔터티를 통해서 조회가능
                        (SELECT COUNT(*)
                        FROM 첨부파일
                        WHERE 게시글번호 = 내가원하는 게시글번호;)
                ==> 쿼리를 매번 이렇게 귀찮아서 게시글 엔터티에 첨부파일 개수라는
                컬럼을 만들고, 첨부파일 수가 수정이 될 때마다 첨부파일, 게시글 엔터티를
                동시에 수정
...
4. 관계 정의
  ...
  관계명은 모델러가 직접 작명을 해야하나 어려운 영역으로 보긴힘듬
...
물리 모델링 : 설계 된 논리 무델링을 시스템에서 사용할 데이터 베이스에 맞게 설계한 모델
            (데이터 타입, 객체명, 컬럼명, 제약조건 - PK, FK)

  학생 정보와 학생이 수강하는 교과목 관리 하려고 한다
  
  학생 정보로는 학번, 학생이름, 생년월일, 집주소, 핸드폰 번호, 집 전화번호를 관리하고
  
  교과목으로는 과목명, 수강 대상학년, 고재정보들을 관리 하려고 한다
  ...
  ex : 사원의 종류가 내근사원, 설계사, 대리점 3개로 구분되고
       ...
       공통 속성은 SuperType에 개별속성은 subType을 통해 관리 할 수 있다
  
Super Type, Sub Type을 물리 모델로 변경하게 될 경우 선택 방안은 3가지가 존재
1. super tpye + 모든 sub Type 통합 (생성되는 테이불 수 : 1)
...
sub type 속성과 super type 속성의 개수가 비슷하고 ==> 3
...
정규화 순서 : 1정규형 - 2정규형 - 3정규형 - BCNF 정규형 - 4정규형 - 5정규형
    ...
    실무에서는 1~3 정규형 까지만 진행, 3정규형 이후 부터는 발생 빈도가 낮아지고, 수고가 커짐
    
함수 종속
a라는 값을 통해 b라는 값이 정해지는 것 ( a -> b)
사번 -> 사원이름, 담당없무, 소속부서, 급여, 매니저
7369 -> SCOTT(brown)
...
집전화번호, 핸드폰번호나 동일한 데이터 타입으로 관리하는게 가능 할거같음. 
