...
1. 인위적으로 여러개의 행을 만들기
   ...
   조인 : 테이블의 행과, 다른 테이블의 행을 연결 - 컬럼이 확장
    ...
    실패 필요한 값 ==> 날짜 (20201001~20201031)
    DATE + 정수 = DATE 에서 정수를 일자로 취급해서 더한 날짜
...
DECLARE
    선언부(생략가능) - pl/sql 블렁게서 사용할 변수, TYPE(CLASS), CURSOR(SQL-정보)등을 선언하는 절
...
PL/SQL PROCEDURE : 오라클 DBMS에 저장한 PL/SQL 블럭
                    함수와 다르게 리턴값이 없다
생성방법
CREATE OR REPLACE PROCEDURE 프로시저명 IS [(입력값....)] IS
    선언부
BEGIN
END;
/ 
...
CREATE OR REPLACE PROCEDURE printemp (p_empno IN emp.empno%TYPE) IS
    ...
    v_dname dept.dname%TYPE;
BEGIN
    SELECT empno, dname INTO v_empno, v_dname
    FROM emp, dept
    WHERE emp.deptno = dept.deptno
    AND emp.empno = p_empno;
    DBMS_OUTPUT.PUT_LINE('v_empno : ' || v_empno || ', v_dname : ' || v_dname);
END;
/
