Exception (예외) : pl/sql 블럭이 실행디는 동안 발생하는 에러
...
BEGIN
    ...
    FROM emp;
EXCEPTION
WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE('NO_DATE_FOUND');
...
SELECT getDeptName(deptno), ename
FROM emp;
...
오라클은 10번인자로 getDeptName 실행 했을 때
ACCOUNTING 이라는 걀과값을 기억(캐싱)
...
borwn c6347b73d5b1f7c77f8be828ee3e871c819578f23779c7d5e082ae2b36a44
SELECT *
FROM users;
...
트리거
    ...
    장점 : users테이블에 비밀번호를 바꾸는 로직(java코드)은 작성을 해야한다
