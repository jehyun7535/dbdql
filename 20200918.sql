...
DDL : Data Defination Languge
    데이터와 관련된 객체를 생성, 수정, 삭제하는명령
...
3. 날짜 : DATE -7 byte고정
            ...
            시스템에서 문자 형식으로 많이 사용한다면 문자 타입으로
            사용도 고려
ranger_no = 1, ranger_nm = 'brown', reg_dt = TO_CHAR(SYSDATE, 'YYYYMMDD');

CREATE TABLE jehyun.ranger
(ranger_no = 1, ranger_nm = 'brown', reg_dt = TO_CHAR(SYSDATE, 'YYYYMMDD'));
...
제약조건 : 데이터에 이상한값이 들어가지 않도록 강제하는 설정
       EX) emp테이블에 empno 컬럼에 값이 없는 상태로 들어가는 것을 방지
           emp테이블에 deptno컬럼의 값이 dept 테이블에 존재하지 않는
           데이터를 입력하는 것을 방지
           emp테이블에 empno(사번) 컬럼의 값이 중복되 않도록 방지
...
90번 부서가 이미 존재 하는 상태 였기 떄문에 PRIMARY KEY 제약에 의해
정상적으로 입력될 수 없다
unique constraint (JEHYUN.SYS_C007110) violated
INSERT INTO dept_test VALUES (90, 'ddit', 'daejeon');

비교
dept 테이블에는 deptno 컬럼에 PRIMARY KEY 제약이 없는상태
...
참조 무결성 제약조건에 의해 emp_test 테이블의 deptno 컬럼의 ㄱ밧은
...
참조 무결성 조건 옵션
1. default : 자식이 잆는 부모 데이터를 삭제할 수 없다
...
1. dept_test테이블의 deptno컬럼에 PRIMARY KEY 제약조건 추가
ALTER TABLE 테이블명 ADD CONSTRAINT 제약조건명 제약조건 타입;
ALTER TABLE 테이블명 DROP CONSTRAINT 제약조건명;
ALTER TABLE dept_test ADD CONSTRAINT PK_DEPT_TEST PRIMARY KEY (deptno);

2. emp_test테이블의 empno컬럼에 PRIMARY KEY 제약조건 추가
ALTER TABLE emp_test ADD CONSTRAINT PK_EMP_TEST PRIMARY KEY (empno);

3. emp_test테이블의 deptno컬럼에 dept_test 컬럼의 deptno 컬럼을
